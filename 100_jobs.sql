...
CREATE FUNCTION app_jobs.update_timestamps ()
	RETURNS TRIGGER
	AS $$
BEGIN
...
CREATE FUNCTION app_jobs.jobs__decrease_job_queue_count ()
	RETURNS TRIGGER
	AS $$
BEGIN
	UPDATE
...
CREATE FUNCTION app_jobs.jobs__increase_job_queue_count ()
	RETURNS TRIGGER
	AS $$
BEGIN
...
CREATE FUNCTION app_jobs.schedule_job (identifier varchar, queue_name varchar, payload json, run_at timestamptz)
	RETURNS app_jobs.jobs
	AS $$
...
CREATE FUNCTION app_jobs.complete_job (worker_id varchar, job_id int)
	RETURNS app_jobs.jobs
	AS $$
DECLARE
	v_row app_jobs.jobs;
...
CREATE FUNCTION app_jobs.fail_job (worker_id varchar, job_id int, error_message varchar)
	RETURNS app_jobs.jobs
	AS $$
DECLARE
	v_row app_jobs.jobs;
...
CREATE FUNCTION app_jobs.get_job (worker_id varchar, identifiers varchar[])
	RETURNS app_jobs.jobs
	AS $$
DECLARE
	v_job_id int;
...
BEGIN
	...
	WHERE (locked_at IS NULL
		OR locked_at < (now() - (coalesce(current_setting('jobs.expiry', TRUE), v_default_job_expiry) || ' seconds')::interval))
		AND run_at <= now()
		AND attempts < coalesce(current_setting('jobs.maximum_attempts', TRUE), v_default_job_maximum_attempts)::int
		AND (identifiers IS NULL
			OR task_identifier = ANY (identifiers))
	ORDER BY
		priority ASC,
		run_at ASC,
		id ASC
